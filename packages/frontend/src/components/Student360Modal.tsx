...
export const Student360Modal: React.FC<Student360ModalProps> = ({
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/exams', { headers })
      .then(r => r.json())
      .then(async body => {
        const exams = body.success ? body.data || [] : [];
        const rows: typeof examRows = [];
        for (const exam of exams.slice(0, 12)) {
          const evRes = await fetch(`/api/v1/exams/${exam.id}/evaluations`, { headers });
          const evBody = await evRes.json();
          const ev = (evBody.data || []).find((e: any) => e.student_id === currentStudent.id);
          if (ev) {
            rows.push({
              title: exam.title,
              date: exam.exam_date,
              obtained: ev.total_obtained,
              total: exam.total_marks,
              grade: ev.grade,
              remarks: ev.short_remarks || ev.long_remarks || '—',
            });
          }
        }
        setExamRows(rows);
    ...
    fetch('/api/v1/homework', { headers })
      .then(r => r.json())
      .then(async body => {
        const hw = body.success ? body.data || [] : [];
        const rows: typeof notebookRows = [];
        for (const item of hw.slice(0, 12)) {
          const cRes = await fetch(`/api/v1/homework/${item.id}/checks`, { headers }).catch(() => null);
          if (!cRes) continue;
          const cBody = await cRes.json();
          const check = (cBody.data || []).find((c: any) => c.student_id === currentStudent.id);
          if (check) {
            rows.push({
